...
Gossiper.prototype.handleGossip = function(connection, message_stream,  message, call) {
	...
	switch(requestType[request_type]){
		case 'GOSSIP_MESSAGE':
			...
			this.logger.debug('Sending from ' + this.port + ' to: ' + connection.remotePort )
			break;
		case 'GOSSIP_ACK':
...
Gossiper.prototype.gossipToPeer = function(peer, message) {
  var a = peer.split(":");
  console.log(peer)
  ...
  gosipee.on('connect', function(net_stream) {
    ...
    mp_stream.on('msg', function(msg) { self.handleGossip(gosipee, mp_stream, msg, 3) });
    this.logger.debug('sendGossip: from: ' + self.port + 'to: ' + peer)
    mp_stream.send(message);
  });
  gosipee.on('error', function(exception) {
   console.log(self.port + " received " + util.inspect(exception));
  });
}

Gossiper.prototype.sendAck = function(host, port, message) {
...
Gossiper.prototype.updateStates = function(deltas) {
	for(var remote_state in deltas){
